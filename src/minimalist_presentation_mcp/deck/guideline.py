...
- If credible numeric data exists, use charts or quantified comparisons inside the capsule. Inline SVG is preferred for charts.
- Supported SVG chart families include bar/column, line, pie/donut, stacked bar, waterfall, scatter, and timeline-like quantitative visuals. Pick the simplest chart that matches the data and label density.
- Keep charts SVG-based inside the capsule. Do not rely on canvas, script-driven chart libraries, or runtime data fetching inside evidence slides.
...
Evidence capsules should usually contain a chart, table, comparison grid, timeline, driver tree, risk map, or similar proof structure. Prefer SVG for charts, including bar/column, line, pie/donut, stacked bar, waterfall, or scatter patterns when appropriate. When charts are used, visible labels should stay compact and full numeric context should be exposed by tooltip text or a companion legend. fallback_text should work as a short takeaway strip.
